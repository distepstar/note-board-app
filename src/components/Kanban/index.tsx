...
export const KanbanSection: React.FC<IKanbanSection> = ({ accept, projectId, titleType, data, handleOnDrop, isLoading }): JSX.Element => {
    ...
    return (
        <Routes>
            <Route path={'/*'} element={
                <div className="kanban-section-wrapper w-3/12 h-[50rem] rounded-md bg-gray-400 pb-4 text-black">
                    <Routes>
                    ...
                    <div className="flex flex-col justify-start place-items-center">
                        ...
                        </div>
                        {
                            isLoading ?
                                <LoadingIcon />
                                :
                                <div className={`kanban-section-items-wrapper overflow-y-scroll no-scrollbar w-full h-[44rem] snap-y ${isOver ? 'bg-gray-500' : ''}`} ref={drop}>
